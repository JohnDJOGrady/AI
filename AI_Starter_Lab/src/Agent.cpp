...
void Agent::update(double dt, Entity &target, std::vector<Agent*> allies)
{
	if (m_behaviour != WANDER)
	{
	...
	if (m_behaviour == PURSUE)
	{
		m_targetVel = target.getVelocity();
		pursue();
	}
...
// INCOMPLETE: wandering is not dynamic, still very jerky
void Agent::wander()
{
	if (m_target != sf::Vector2f() && distance(m_target, m_pos) > 100)
	{
		...
		wander = sf::Vector2f(-sin(m_orientation * TO_RAD), cos(m_orientation * TO_RAD)) * MAX_SPEED;
...
{
	sf::Vector2f target = m_target - m_pos;
	if (mag(target) > OUTER_ARRIVAL_RAD)
	{
...
void Agent::pursue()
{
	sf::Vector2f direction = m_target - m_pos;
	float timePrediction;
	float distance = mag(direction);
	float speed = mag(m_vel);
	if (speed <= distance / TIME_TO_TARGET)
	{
		timePrediction = TIME_TO_TARGET;
	}
	else
	{
		timePrediction = distance / speed;
	}
	m_target = m_target + m_targetVel * timePrediction;
	arrive();
}

bool Agent::lookAhead(sf::Vector2f target)
{
