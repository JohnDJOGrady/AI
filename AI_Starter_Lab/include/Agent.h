...
static const float TIME_TO_TARGET = .75f;
...
static const float MAX_STEER = .5f;

static const float AVOID_CONE = 70.0f;
static const float COLL_RAD = 100.f;
...
public:
	...
	void update(double dt, Entity &target, std::vector<Agent*> allies);
	...
	bool lookAhead(sf::Vector2f target);
